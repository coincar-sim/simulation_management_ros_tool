...
TimeMgmt::TimeMgmt(ros::NodeHandle node_handle,
	...
	reconfigSrv_.setCallback(
			boost::bind(&TimeMgmt::reconfigureRequest, this, _1, _2));

	/**
	 * Set up publisher & timer
	 */
	clockPub_ = private_node_handle.advertise<rosgraph_msgs::Clock>("/clock",
			5);
	timer_ = private_node_handle.createWallTimer(
			ros::WallDuration(
					1.0 / params_.time_resolution / params_.acc_factor),
			&TimeMgmt::timerCallbackSim, this);
...
void TimeMgmt::timerCallbackSim(const ros::WallTimerEvent&) {
	ros::WallDuration currentRealTimeDifference = ros::WallTime::now()
			- startWallTime_;
	ros::Duration currentSimTimeDifference;
	currentSimTimeDifference = currentSimTimeDifference.fromNSec(
			currentRealTimeDifference.toNSec() * params_.acc_factor);
